...
class MatrixImpl<E>(override val height: Int, override val width: Int) : Matrix<E> {
    var matrix: MutableList<E> = mutableListOf<E>()
    ...
    override fun get(row: Int, column: Int): E {
        ...
        return matrix[row*this.height + column];
    }
    ...
    override fun set(row: Int, column: Int, value: E) {
        matrix[row*this.height + column] = value
    }
    ...
    override fun equals(other: Any?): Boolean {
        ...
        for (i in 0 until this.height)
            for (j in 0 until this.width)
                if (this.matrix[i*this.height + j] != other.matrix[i*this.height + j])
                    return false
    ...
    override fun toString(): String {
        ...
        for (i in 0 until height) {
            ...
            for (j in 0 until width) {
                res.append(matrix[i*this.height + j])
                res.append(' ')
            }
            if (i != height -1 )
                res.append('\n')
